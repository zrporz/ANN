...
def train_net(model, loss, config, inputs, labels, batch_size, disp_freq, log):
    ...
    for input, label in data_iterator(inputs, labels, batch_size):
        ...
        if iter_counter % disp_freq == 0:
            ...
            log.append("train_loss",np.mean(loss_list))
            log.append("train_acc",np.mean(acc_list))
            loss_list = []
            acc_list = []
            LOG_INFO(msg)


def test_net(model, loss, inputs, labels, batch_size,log):
    loss_list = []
    acc_list = []
    ...
    log.append("test_loss",np.mean(loss_list))
    log.append("test_acc",np.mean(acc_list))
    LOG_INFO(msg)
