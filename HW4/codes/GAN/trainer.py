...
class Trainer(object):
    ...
    def train_step(self, real_imgs, fake_imgs, BCE_criterion):
        ...
        *   Returns:
        ...
        # TODO START
        D_real_prob = self._netD(real_imgs)
        D_real_label = torch.ones_like(D_real_prob)		
        loss_D_real = BCE_criterion(D_real_prob,D_real_label)
        D_x = D_real_prob.mean()
        loss_real = loss_D_real
        loss_real.backward()
        # TODO END
        ...
        # TODO START
        D_fake_prob = self._netD(fake_imgs)
        D_fake_label = torch.zeros_like(D_fake_prob)
        loss_D_fake = BCE_criterion(D_fake_prob,D_fake_label)
        D_G_z1 = D_fake_prob.mean()
        loss_fake = loss_D_fake
        ...
        # record average D(fake_imgs)
        # TODO START
        G_fake_prob = self._netD(fake_imgs)
        G_fake_label = torch.ones_like(G_fake_prob)
        loss_G = BCE_criterion(G_fake_prob,G_fake_label)
        D_G_z2 = G_fake_prob.mean()
