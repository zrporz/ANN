...
class Generator(nn.Module):
    def __init__(self, num_channels, latent_dim, hidden_dim):
        ...
        self.decoder = nn.Sequential(
            # layer1 (latent-dim,1,1) -> (4*hidden-dim,4,4)
            nn.ConvTranspose2d(latent_dim, hidden_dim * 4, 4, 1, 0), 
            nn.BatchNorm2d(hidden_dim * 4),
            nn.ReLU(),
            # layer2 (4*hidden-dim,4,4) -> (2*hidden-dim,8,8)
            nn.ConvTranspose2d(hidden_dim * 4, hidden_dim * 2, 4, 2, 1),
            nn.BatchNorm2d(hidden_dim * 2),
            nn.ReLU(),
            # layer3 (2*hidden-dim,8,8) -> (hidden-dim,16,16)
            nn.ConvTranspose2d(hidden_dim * 2, hidden_dim, 4, 2, 1),
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(),
            # layer4 (hidden-dim,16,16) -> (1,32,32)
            nn.ConvTranspose2d(hidden_dim, 1, 4, 2, 1),
            nn.Tanh(),
